...
class newnode:
    def __init__(self, node, parent, pparent,ppparent,pppparent,ppppparent):
        self.node = node
        self.parent = parent
        self.pparent = pparent
        self.ppparent = ppparent
        self.pppparent = pppparent
        self.ppppparent = ppppparent
class Graph():
    """
...
#        for fpFunc, callerSet in fpFuncToCaller.items():
    ...
    def getLeavesFromStartNode(self, nodeName, filterList, exceptList,binName):
        results = set()
        visitedNodes = set()
        myStack = list()
        tmp=newnode(nodeName,None,None,None,None,None)
        myStack.append(tmp)
        file=open("./g_ouput/mapping_"+binName,"a+")
        if ( len(self.adjGraph.get(nodeName, list())) == 0 ):
            return results

        while ( len(myStack) != 0 ):
            currentNode = myStack.pop()
            if ( currentNode.node not in visitedNodes):
                #self.logger.debug("Visiting node: " + currentNode)
                visitedNodes.add(currentNode.node)
                if ( len(self.adjGraph.get(currentNode.node, list())) != 0 ):
                    for node in self.adjGraph.get(currentNode.node, list()):
                        tmp=newnode(node,currentNode.node,currentNode.parent,currentNode.pparent,currentNode.ppparent,currentNode.pppparent)
                        myStack.append(tmp)
                else:
                    if ( ( len(filterList) == 0 and len(exceptList) == 0 ) or ( len(filterList) > 0 and currentNode.node in filterList) or ( len(exceptList) > 0 and currentNode.node not in exceptList ) ):
                        results.add(currentNode.node)
                        currentNode1 = currentNode.node.replace("syscall","")
                        currentNode1 = currentNode1.replace("(","")
                        currentNode1 = currentNode1.replace(")","")
                        currentNode1 = currentNode1.strip()
                        if ( not currentNode1.startswith("%") ):
                            results.add(int(currentNode1))
                                                      
                            file.write(str(nodeName)+"->")
                            file.write(str(currentNode.ppppparent)+"->")
                            file.write(str(currentNode.pppparent)+"->")
                            file.write(str(currentNode.ppparent)+"->")
                            file.write(str(currentNode.pparent)+"->")
                            file.write(str(currentNode.parent)+"->")
                            #file.write(str(currentNode1)+"::")
                            syscall_name=sysmap[int(currentNode1)]
                            file.write(syscall_name)
                            file.write("\n")
...
#                self.logger.debug("Visiting node: " + currentNode)
                ...
                if ( len(self.adjGraph.get(currentNode.node, list())) != 0 ):
                    for node in self.adjGraph.get(currentNode.node, list()):
                        tmp=newnode(node,currentNode.node)
                        myStack.append(tmp)
                else:
                    if ( currentNode.node.strip().startswith("syscall") ):
